...
function AITabContent({
  ...
  const folder = useDashboardStore((s) => s.folders.find((f) => f.id === String(h.folderId)));
        ...
        <div className="space-y-2">
          ...
          {canEdit ? (
            ...
            >
             ...
             <span className="text-[10px] text-white/20 italic">Generated by owner</span>
          )}
        </div>
      ) : (
        <div className="flex items-center justify-between gap-2">
          <p className="text-[11px] text-white/35 italic">
            {canEdit ? placeholder : "Ask owner to generate insights"}
          </p>
          <button
            onClick={onGenerate}
            disabled={!canEdit}
